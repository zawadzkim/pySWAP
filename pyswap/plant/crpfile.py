...
Similar to the .dra or .swp files, the .crp file is a configuration file for
the SWAP model. The classes in this module represent distincs sections of
the .crp file. The main class is the `CropFile` class which holds
the settings for the crop simulation.

Classes:
    CropFile: Class for the .crp file.
    CropDevelopmentSettings: Base class for crop development settings.
    CropDevelopmentSettingsWOFOST: Crop development settings
        for WOFOST module.
    CropDevelopmentSettingsFixed: Fixed crop development
        settings.
    OxygenStress: Oxygen stress settings.
    DroughtStress: Drought stress settings.
    SaltStress: Salt stress settings.
    CompensateRWUStress: Root water uptake compensation settings.
    Interception: Crop interception settings.
    CO2Correction: CO2 correction settings.
    ScheduledIrrigation: Scheduled irrigation settings.
    Preparation: Settings related to field preparation before sowing.
"""
...
class CropDevelopmentSettings(PySWAPBaseModel, SerializableMixin):
    ...
    Attributes:
        ...
        table_dvs_cf (Optional[Table]): Table with crop factors as a function
            of development stage
        table_dvs_ch (Optional[Table]): Table with crop height as a function
            of development stage
        ...
        rdtb (Optional[Arrays]): Rooting Depth as a function of
            development stage
        ...
        swdmi2rd (Optional[Literal[0, 1]]): Switch for calculation
            rooting depth

            * 0 - Rooting depth increase is related to availability
                assimilates for roots
            * 1 - Rooting depth increase is related to relative
                dry matter increase
    ...
    def _validate_crop_base(self) -> Self:
        if self.swcf == 1:
            assert (
                self.table_dvs_cf is not None
            ), "table_dvs_cf is required when swcf is 1."
        elif self.swcf == 2:
            assert (
                self.table_dvs_ch is not None
            ), "table_dvs_ch is required when swcf is 2."
            assert (
                self.albedo is not None
            ), "albedo is required when swcf is 2."
        ...
        if self.swrd == 1:
            ...
            assert (
                self.swdmi2rd is not None
            ), "swdmi2rd is required when swrd is 2."
        elif self.swrd == 3:
            assert self.rlwtb is not None, "rlwtb is required when swrd is 3."
            assert (
                self.wrtmax is not None
            ), "wrtmax is required when swrd is 3."

        return self


class CropDevelopmentSettingsWOFOST(CropDevelopmentSettings):
    """Additional crop settings for the WOFOST module.
    ...
    idsl: Optional[Literal[0, 1, 2]] = None
    dtsmtb: Optional[Arrays] = None
    ...
    tmpftb: Arrays
    tmnftb: Arrays
    cvo: Optional[float] = Field(**UNITRANGE, default=None)
    ...
    rmo: Optional[float] = Field(**UNITRANGE, default=None)
    ...
    def _validate_crop_wofost(self) -> Self:
        if self.idsl in [1, 2]:
            assert (
                self.dlc is not None
            ), "dlc is required when idsl is either 1 or 2."
            assert (
                self.dlo is not None
            ), "dlo is required when idsl is either 1 or 2."
        elif self.idsl == 2:
            assert (
                self.vernsat is not None
            ), "vernsat is required when idsl is 2."
            assert (
                self.vernbase is not None
            ), "vernbase is required when idsl is 2."
            assert (
                self.verndvs is not None
            ), "verndvs is required when idsl is 2."
            assert (
                self.verntb is not None
            ), "verntb is required when idsl is 2."
...
class CropDevelopmentSettingsFixed(CropDevelopmentSettings):
    ...
    Attributes:
        ...
        swgc (Literal[1, 2]): Choose between Leaf Area Index or
            Soil Cover Fraction
    ...
    def _validate_crop_fixed(self) -> Self:
        if self.idev == 1:
            ...
            assert (
                self.tsumea is not None
            ), "tsumea is required when idev is 2."
            assert (
                self.tsumam is not None
            ), "tsumam is required when idev is 2."
...
class CropDevelopmentSettingsGrass(CropDevelopmentSettingsWOFOST):
    ...
    Attributes:
        swtsum (Literal[0, 1, 2]): Select either sum air temperatures or soil
            temperature at particular depth

            * 0 - no delay of start grass growth
            * 1 - start of grass growth based on sum air
                temperatures > 200 degree C
            * 2 - start of grass growth based on soil temperature at
                particular depth

        tsumtemp (Optional[float]): Specific stem area [0..1 ha/kg, R]
        tsumdepth (Optional[float]): Life span under leaves under optimum
            conditions [0..366 d, R]
        tsumtime (Optional[float]): Lower threshold temperature for ageing of
            leaves [-10..30 degree C, R]
    """
...
class OxygenStress(PySWAPBaseModel, SerializableMixin):
    ...
    Attributes:
        ...
        swoxygentype (Optional[Literal[1, 2]]): switch for physical processes
            or repro. functions to calculate oxygen stress
        ...
        aeratecrit (Optional[float]): Threshold to stop root extension in case
            of oxygenstress; 0.0 maximum oxygen stress
        hlim1 (Optional[float]): No water extraction at higher pressure heads
        hlim2u (Optional[float]): H below which optimum water extr. starts for
            top layer
        hlim2l (Optional[float]): H below which optimum water extr. starts for
            sub layer
        q10_microbial (Optional[float]): Relative increase in microbial
            respiration at temperature increase of 10 C
        specific_resp_humus (Optional[float]): Respiration rate of humus
            at 25 C
        srl (Optional[float]): Specific root length
        swrootradius (Optional[Literal[1, 2]]): Switch for calculation of
            root radius
        ...
        spec_weight_root_tissue (Optional[float]): Specific weight of
            non-airfilled root tissue
        var_a (Optional[float]): Variance of root radius
        root_radiuso2 (Optional[float]): Root radius for oxygen stress module
        q10_root (Optional[float]): Relative increase in root respiration at
            temperature increase of 10 oC
        f_senes (Optional[float]): Reduction factor for senescence, used for
            maintenance respiration
        c_mroot (Optional[float]): Maintenance coefficient of root
        table_max_resp_factor (Optional[Table]): Ratio root total
            respiration / maintenance respiration as a function of
            development stage
        table_dvs_w_root_ss (Optional[Table]): List dry weight of roots at
            soil surface as a function of development stage\
    ...
    def _validate_oxygen(self) -> Self:
        if self.swoxygen == 1:
            assert (
                self.hlim1 is not None
            ), "hlim1 is required when swoxygen is 1."
            assert (
                self.hlim2u is not None
            ), "hlim2u is required when swoxygen is 1."
            assert (
                self.hlim2l is not None
            ), "hlim2l is required when swoxygen is 1."
        elif self.swoxygen == 2:
            assert (
                self.q10_microbial is not None
            ), "q10_microbial is required when swoxygen is 2."
            assert (
                self.specific_resp_humus is not None
            ), "specific_resp_humus is required when swoxygen is 2."
            assert self.srl is not None, "srl is required when swoxygen is 2."
            assert (
                self.swrootradius is not None
            ), "swrootradius is required when swoxygen is 2."
            if self.swrootradius == 1:
                assert (
                    self.dry_mat_cont_roots is not None
                ), "dry_mat_cont_roots is required when swrootradius is 1."
                assert (
                    self.air_filled_root_por is not None
                ), "air_filled_root_por is required when swrootradius is 1."
                assert (
                    self.spec_weight_root_tissue is not None
                ), "spec_weight_root_tissue is required when "
                "swrootradius is 1."
                assert (
                    self.var_a is not None
                ), "var_a is required when swrootradius is 1."
            elif self.swrootradius == 2:
                assert (
                    self.root_radiuso2 is not None
                ), "root_radiuso2 is required when swrootradius is 2."
        if self.swwrtnonox == 1:
            assert (
                self.aeratecrit is not None
...
class DroughtStress(PySWAPBaseModel, SerializableMixin):
    ...
    Attributes:
        ...
        swjarvis (Optional[Literal[0, 1, 2, 3, 4]]): _DEPRECATED_ Switch for
            Jarvis model for water uptake reduction
        alphcrit: Optional[float] = _DEPRECATED_ Critical stress index
            (Jarvis, 1989) for compensation of root water uptake [0.2..1 -, R]
        hlim3h (Optional[float]): Pressure head below which water uptake
            reduction starts at high Tpot
        hlim3l (Optional[float]): Pressure head below which water uptake
            reduction starts at low Tpot
        hlim4 (Optional[float]): No water extraction at lower soil water
            pressure heads
        adcrh (Optional[float]): Level of high atmospheric demand,
            corresponding to HLIM3H
        adcrl (Optional[float]): Level of low atmospheric demand,
            corresponding to HLIM3L
        wiltpoint (Optional[float]): Minimum pressure head in leaves
        kstem (Optional[float]): Hydraulic conductance between leaf and
            root xylem
        ...
        rootcoefa (Optional[float]): Defines relative distance between roots
            at which mean soil water content occurs
        swhydrlift (Optional[Literal[0, 1]]): Switch for possibility hydraulic
            lift in root system
        rooteff (Optional[float]): Root system efficiency factor
        stephr (Optional[float]): Step between values of hroot and hxylem in
            iteration cycle
        criterhr (Optional[float]): Maximum difference of Hroot between
            iterations; convergence criterium
        taccur (Optional[float]): Maximum absolute difference between
            simulated and calculated potential transpiration rate
    """
    ...
    def _validate_prepartion(self) -> Self:
        if self.swdrought == 1:
            assert (
                self.hlim3h is not None
            ), "hlim3h is required when swdrought is 1."
            assert (
                self.hlim3l is not None
            ), "hlim3l is required when swdrought is 1."
            assert (
                self.hlim4 is not None
            ), "hlim4 is required when swdrought is 1."
            assert (
                self.adcrh is not None
            ), "adcrh is required when swdrought is 1."
            assert (
                self.adcrl is not None
            ), "adcrl is required when swdrought is 1."
        if self.swdrought == 2:
            assert (
                self.wiltpoint is not None
            ), "wiltpoint is required when swdrought is 2."
            assert (
                self.kstem is not None
            ), "kstem is required when swdrought is 2."
            assert (
                self.rxylem is not None
            ), "rxylem is required when swdrought is 2."
            assert (
                self.rootradius is not None
            ), "rootradius is required when swdrought is 2."
            assert (
                self.kroot is not None
            ), "kroot is required when swdrought is 2."
            assert (
                self.rootcoefa is not None
            ), "rootcoefa is required when swdrought is 2."
            assert (
                self.swhydrlift is not None
            ), "swhydrlift is required when swdrought is 2."
            assert (
                self.rooteff is not None
            ), "rooteff is required when swdrought is 2."
            assert (
                self.stephr is not None
            ), "stephr is required when swdrought is 2."
            assert (
                self.criterhr is not None
            ), "criterhr is required when swdrought is 2."
            assert (
                self.taccur is not None
            ), "taccur is required when swdrought is 2."
...
class SaltStress(PySWAPBaseModel, SerializableMixin):
    ...
    Attributes:
        ...
        saltslope (Optional[float]): Decline of root water uptake
            above threshold
        salthead (Optional[float]): Conversion factor salt concentration
            (mg/cm3) into osmotic head (cm)
    """
    ...
    def _validate_prepartion(self) -> Self:
        if self.swsalinity == 1:
            assert (
                self.saltmax is not None
            ), "saltmax is required when swsalinity is 1."
            assert (
                self.saltslope is not None
            ), "saltslope is required when swsalinity is 1."
        elif self.swsalinity == 2:
            assert (
                self.salthead is not None
...
class CompensateRWUStress(PySWAPBaseModel, SerializableMixin):
    ...
    Attributes:
        swcompensate (Literal[0, 1, 2]): Switch for compensate
        ...
        alphacrit (Optional[float]): Critical stress index for compensation of
            root water uptake
        dcritrtz (Optional[float]): Threshold of rootzone thickness after
...
class Interception(PySWAPBaseModel, SerializableMixin):
    ...
    Attributes:
        ...
        cofab (Optional[float]): Interception coefficient, corresponding to
            maximum interception amount
        table_intertb (Optional[Table]): table with the following columns as a
            function of time T:
            ...
            * AVEVAP = Average evaporation intensity during rainfall from a
                wet canopy
    """
    ...
    def _validate_prepartion(self) -> Self:
        if self.swinter == 1:
            assert (
                self.cofab is not None
            ), "cofab is required when swinter is 1."
        elif self.swinter == 1:
            assert (
                self.table_intertb is not None
...
class CO2Correction(PySWAPBaseModel, SerializableMixin):
    ...
    Attributes:
        swco2 (Literal[0, 1]): Switch for assimilation correction due to
            CO2 impact
        ...
        co2amaxtb (Optional[Arrays]): Correction of photosynthesis as a
            function of atmospheric CO2 concentration
        co2efftb (Optional[Arrays]): orrection of radiation use efficiency as
            a function of atmospheric CO2 concentration
        co2tratb (Optional[Arrays]): Correction of transpiration as a function
    ...
    def _validate_co2correction(self) -> Self:
        if self.swco2 == 1:
            assert (
                self.atmofil is not None
            ), "amofil is required when swco2 is 1"
            assert (
                self.co2amaxtb is not None
            ), "co2amaxtb is required when swco2 is 1"
            assert (
                self.co2efftb is not None
            ), "co2efftb is required when swco2 is 1"
            assert (
                self.co2tratb is not None
            ), "co2tratb is required when swco2 is 1"
...
class Preparation(PySWAPBaseModel, SerializableMixin):
    ...
    Attributes:
        ...
        zprep (Optional[float]): Z-level for monitoring work-ability for
            the crop
        hprep (Optional[float]): Maximum pressure head during preparation
        maxprepdelay (Optional[int]): Maximum delay of preparation from start
            of growing season
        zsow (Optional[float]): Z-level for monitoring work-ability for
            the crop
        hsow (Optional[float]): Maximum pressure head during sowing
        ztempsow (Optional[float]): Z-level for monitoring temperature
            for sowing
        tempsow (Optional[float]): Soil temperature needed for sowing
        maxsowdelay (Optional[int]): Maximum delay of sowing from start of
            growing season
        tsumemeopt (Optional[float]): Temperature sum needed for crop emergence
        tbasem (Optional[float]): Minimum temperature, used for germination
            trajectory
        teffmx (Optional[float]): Maximum temperature, used for germination
            trajectory
        hdrygerm (Optional[float]): Pressure head rootzone for dry germination
            trajectory
        hwetgerm (Optional[float]): Pressure head rootzone for wet germination
            trajectory
    ...
    def _validate_prepartion(self) -> Self:
        if self.swprep == 1:
            assert (
                self.zprep is not None
            ), "zprep is required when swprep is 1."
            assert (
                self.hprep is not None
            ), "hprep is required when swprep is 1."
            assert (
                self.maxprepdelay is not None
        ...
        if self.swsow == 1:
            ...
            assert (
                self.ztempsow is not None
            ), "ztempsow is required when swsow is 1."
            assert (
                self.tempsow is not None
            ), "tempsow is required when swsow is 1."
            assert (
                self.maxsowdelay is not None
            ), "maxsowdelay is required when swsow is 1."

        if self.swgerm in (1, 2):
            assert (
                self.tsumemeopt is not None
            ), "tsumemeopt is required when swgerm is 1 or 2."
            assert (
                self.tbasem is not None
            ), "tbasem is required when swgerm is 1 or 2."
            assert (
                self.teffmx is not None
            ), "teffmx is required when swgerm is 1 or 2."
        elif self.swgerm == 2:
            assert (
                self.hdrygerm is not None
            ), "hdrygerm is required when swgerm is 2."
            assert (
                self.hwetgerm is not None
            ), "hwetgerm is required when swgerm is 2."
            assert (
                self.zgerm is not None
            ), "zgerm is required when swgerm is 2."
            assert (
                self.agerm is not None
            ), "agerm is required when swgerm is 2."
...
class GrasslandManagement(PySWAPBaseModel, SerializableMixin):
    ...
    Attributes:
        seqgrazmow (IntList): sequence of periods with different practices
            within calender year. Available options:

            * 1 - Grazing
            * 2 - Mowing
            * 3 - Grazing with dewooling

        swharvest (Literal[1, 2]): Switch for timing harvest, either for
            mowing or grazing
        ...
        swdmgrz Optional[(Literal[1, 2])]: Switch for dry matter threshold to
            trigger harvest by grazing

            * 1 - Use fixed threshold
            * 2 - Use flexible threshold

        dmgrazing Optional[(Arrays)]: Minimum dry matter amount for cattle to
            enter the field [0..1d6 kg DM/ha, R]
        dmgrztb Optional[(int)]: List threshold of above ground dry matter
            [0..1d6 kg DM/ha, R] to trigger grazing as function of
            daynumber [1..366 d, R]
        maxdaygrz Optional[(int)]: Maximum growing period after harvest
            [1..366 -, I]
        swlossgrz Optional[(Literal[0, 1])]: Switch for losses due to
            insufficient pressure head during grazing

            * 0 - No loss
            * 1 - Losses due to treading

        tagprest Optional[(float)]: Minimum amount of above ground DM after
            grazing [0..1d6 kg DM/ha, R]
        dewrest Optional[(float)]: Remaining yield above ground after
            dewooling event [0..1d6 kg DM/ha, R]
        table_lsda (Optional[Table]): Actual livestock density of each grazing
            period
        table_lsdb (Optional[Table]): Relation between livestock density,
            number of grazing days and dry matter uptake
        swdmmow Optional[(int)]: Switch for dry matter threshold to trigger
            harvest by mowing

            * 1 - Use fixed threshold
            * 2 - Use flexible threshold

        dmharvest Optional[(float)]: Threshold of above ground dry matter to
            trigger mowing [0..1d6 kg DM/ha, R]
        daylastharvest Optional[(int)]: Last calendar day on which mowing may
            occur [1..366 -, I]
        dmlastharvest Optional[(float)]: Minimum above ground dry matter for
            mowing on last date [0..1d6 kg DM/ha, R]
        dmmowtb Optional[(int)]: Dry matter mowing threshold
        maxdaymow Optional[(int)]:Maximum growing period after harvest
            [1..366 -, I]
        swlossmow Optional[(int)]: Switch for losses due to insufficient
            pressure head during mowing

            * 0 - No loss
            * 1 - Losses due to treading

        mowrest Optional[(float)]: Remaining yield above ground after mowing
            event [0..1d6 kg DM/ha, R]
        table_dmmowdelay Optional[(Optional[Table])]: Relation between dry
            matter harvest [0..1d6 kg/ha, R] and days of delay in regrowth
            [0..366 d, I] after mowing
        swpotrelmf (int): Switch for calculation of potential yield

            * 1 - theoretical potential yield
            * 2 - attainable yield

        relmf (float): Relative management factor to reduce theoretical
            potential yield to attainable yield [0..1 -, R]
    """
    ...
    def _validate_grassland_management(self) -> Self:
        if self.swharvest == 2:
            assert (
                self.dateharvest is not None
            ), "dateharvest is required when swharvest is 2."
        if self.swdmgrz == 1:
            assert (
                self.dmgrazing is not None
            ), "dmgrazing is required when swdmgrz is 1."
            assert (
                self.dmgrztb is not None
            ), "dmgrztb is required when swdmgrz is 1."
            assert (
                self.maxdaygrz is not None
            ), "maxdaygrz is required when swdmgrz is 1."
        if self.swdmgrz == 2:
            assert (
                self.tagprest is not None
            ), "tagprest is required when swdmgrz is 2."
            assert (
                self.dewrest is not None
            ), "dewrest is required when swdmgrz is 2."
            assert (
                self.table_lsda is not None
            ), "table_lsda is required when swdmgrz is 2."
...
class CropFile(PySWAPBaseModel, FileMixin):
    ...
    This class collects all the settings for the crop file. Currently the
    types of the attributes are set to Any because the validation is not
    yet implemented.
    ...
    Attributes:
        ...
        compensaterwu (Optional[CompensateRWUStress]): Compensate root water
            uptake stress settings
        interception (Optional[Interception]): Interception settings
        scheduledirrigation (Optional[ScheduledIrrigation]): Scheduled
            irrigation settings
        grassland_management (Optional[GrasslandManagement]): Grassland
            management settings
    """
