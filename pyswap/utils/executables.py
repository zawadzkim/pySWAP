...
def _download_swap_executable(
    ...
    Note: On Windows, only the MinGW cross-compiled version is downloaded
    because the Intel Fortran version has unresolved DLL dependencies.
    """
    # Construct download URL
    base_url = "https://github.com/SWAP-model/SWAP/releases/download"
    # Use MinGW version on Windows (statically linked, no DLL dependencies)
    # Use native Linux version on Linux
    filename = f"swap{version}-mingw.exe" if IS_WINDOWS else f"swap{version}-linux"
