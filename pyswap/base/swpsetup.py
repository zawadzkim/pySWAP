...
# database models
from pySWAP.utils_database.models import SWAPModel, ModelIteration
from pySWAP.utils import exeptions as exc
from pySWAP.utils.utils import list_to_string
# Submodules responsible for files outside the swp file
from .crop import Crop  # crop parameters
from .drainage import Drainage  # drainage parameters
from .metadata import Metadata  # project metadata
from .meteo import Meteo  # meteo data
from utils_database.connection import DatabaseConnection  # database connection

"""sets up the swp model"""


@dataclass
class SWAPSetup:
    """Contains methods for manipulating SWAPModel objects.

    Most of the methods are meant to update the swp field of the SWAPModel object. The swp field is a JSON field
    containing the contents of the swp file in the form of key-value pairs. The swp file is the main text file
    required to run the SWAP model.
    """

    connection: DatabaseConnection = field(repr=False)
    metadata: Metadata = field(repr=False)
    model: SWAPModel = field(default=None, repr=False)
    meteo: Meteo = field(default=None, repr=False)
    crop: Crop = field(default=None, repr=False)
    drainage: Drainage = field(default=None, repr=False)
    swp_updates: dict = field(default_factory=dict, repr=False)

    def __post_init__(self):
        # Update header info
        self._set_header()

        self.model = self.connection.session.query(SWAPModel).filter_by(name=self.metadata.project_name).first()
        if self.model is None:
            self.model = SWAPModel(name=self.metadata.project_name, swp={}, drainage={}, crop={}, met=None)
            self.connection.session.add(self.model)
            self.connection.session.commit()

        # Crude solution, but it works.
        model_iterations_count = (
            self.connection.session.query(ModelIteration)
            .filter_by(model_id=self.model.id)
            .count()
        )
        ...
        if model_iterations_count == 0:
            ...
            self.connection.session.add(model_iteration)
            self.connection.session.commit()
    ...
    def meteo_to_binary(self):
        # Convert the DataFrame to bytes
        df = self.meteo.meteodata
        binary_file = io.BytesIO()
        df.to_feather(binary_file)
        binary_file.seek(0)  # Reset the file position to the beginning

        return binary_file.getvalue()

    def save(self):

        self.model.swp = self.swp_updates
        self.model.drainage = self.drainage.file
        self.model.crop = self.crop.files
        self.model.met = self.meteo_to_binary()

        # Save the updated model to the database
        self.connection.session.add(self.model)
        self.connection.session.commit()

        # Reset the swp_updates dictionary
        self.swp_updates = {}

    def _get_args_dict(self, locals_dict: dict) -> dict:
        ...
        excluded_types = [Crop, Drainage, Meteo]
        args_dict = {}
        for key, value in locals_dict.items():
            if key in parameters and key != 'self' and not any(isinstance(value, t) for t in excluded_types):
                if isinstance(value, DataFrame):
                    args_dict['TABLE_' + key.upper()] = value.to_string(index=False)
                else:
                    args_dict[key.upper()] = value

        return args_dict

    def _update_swp(self, parameters: dict, section: str):
        if section not in self.swp_updates.keys():
            self.swp_updates[section] = {}

        self.swp_updates[section].update(parameters)
    ...
    def _set_header(self) -> None:
        ...
        heder_params = {'PROJECT': self.metadata.project_name,
        ...
        self._update_swp(heder_params, 'general')

    def set_environment(self, pathwork: str = '.\\', pathatm: str = '.\\',
                        pathcrop: str = '.\\', pathdrain: str = '.\\',
                        swscre: int = 0, swerror: bool = False):

        self._update_swp(self._get_args_dict(locals()), 'general')
    ...
    def set_output_dates(self,
        ...
        if not swmonth:
            ...
            if swodat:
                if outdatin is None:
                    raise exc.ErrorMissingParameter('When SWODAT is True, OUTDATIN has to be provided.')
                else:
                    outdatin = DataFrame({'date': outdatin}).to_string(header=False, index=False)

        if not swyrvar:
            if datefix is None:
                raise exc.ErrorMissingParameter('If SWYRWAR is False, DATEFIX has to be specified.')
        else:
            if outdat is None:
                raise exc.ErrorMissingParameter('If SWYRVAR is True, OUTDAT has to be specified.')
            else:
                outdat = DataFrame({'date': outdat}).to_string(header=False, index=False)
    ...
    def set_output_files(self,
        ...
        if swcsv:
            if inlist_csv is None:
                raise exc.ErrorMissingParameter('If SWCSV is True, INLIST_CSV has to be provided.')
            else:
        ...
        if swcsv_tz:
            if inlist_csv_tz is None:
                raise exc.ErrorMissingParameter('If SWCSV_TZ is True, INLIST_CSV_TZ has to be provided.')
            else:
        ...
        if swafo > 0 or swaun > 0:
            if critdevmasbal is None or swdiscrvert is None:
                ...
                exc.check_range(critdevmasbal, 0.0, 1.0, left='closed', right='closed')
    ...
    def set_crop(self,
        ...
        self._update_swp(self._get_args_dict(locals()), 'crop')
    ...
    def set_irrigation(self,
        ...
        if swirfix:
            if swirgfil is None:
                raise exc.ErrorMissingParameter('If SWIRGFIX is True, SWIRGFIL cannot be empty.')
        ...
        if schedule:
            ...
            if any(element is None for element in required):
                ...
                exc.check_range(phFieldCapacity, -1000, 0, left='closed', right='closed')
                exc.check_range(tcs, 1, 6, left='closed', right='closed')

            if tcs == 1:
                dvs_tc1 = DataFrame({'dvs_tc1': [0.0, 2.0],
                                        'Trel': [0.95, 0.95]})
            elif tcs == 2:
                dvs_tc2 = DataFrame({'dvs_tc2': [0.0, 2.0],
                                        'RAW': [0.95, 0.95]})
            elif tcs == 3:
                dvs_tc3 = DataFrame({'dvs_tc3': [0.0, 2.0],
                                        'TAW': [0.50, 0.50]})
            elif tcs == 4:
                dvs_tc4 = DataFrame({'dvs_tc4': [0.0, 2.0],
                                        'DWA': [0.40, 0.40]})
            elif dvs_tc5 == 5:
                ...
                if any(element is None for element in required is None):
                    ...
                    dvs_tc5 = DataFrame({'dvs_tc5': [0.0, 2.0],
                                            'Value_tc5': [-1000.0, -1000.0]})
                    if swcirrthres:
                        exc.check_type(cirrthres, float)
                        exc.check_range(cirrthres, 0, 100, left='closed', right='closed')
                        exc.check_type(perirrsurp, float)
                        exc.check_range(perirrsurp, 0, 100, left='closed', right='closed')

            elif tcs == 6:
                exc.check_type(irgthreshold, float)
                exc.check_range(irgthreshold, 0, 20, left='closed', right='closed')
                exc.check_type(tcsfix, int)

                if tcsfix == 1:
                    exc.check_type(irgdayfix, int)
                    exc.check_range(irgdayfix, 1, 366, left='closed', right='closed')

        self._update_swp(self._get_args_dict(locals()), 'irrigation')
    ...
    def set_meteo(self,
                  ...
                  swdivide: bool | None = None,  # apply P-M for distribution? else use crop factors.
                  swmetdetail: bool | None = None,
                  nmetdetail: int | None = None,
                  swetsine: bool = False,  # only required with swetr = False, but model does not run without it
        ...
        if swetr:  # if P-M method is to be used:
            ...
            exc.check_range(meteo_data.met_file_params['{ALT}'], -400, 3000, left='closed', right='closed')
            ...
            if swmetdetail:
                if nmetdetail is None:
                    ...
                    exc.check_range(nmetdetail, 1, 96, left='closed', right='closed')
        ...
        meteo_params = {'METFIL': f"meteo_{meteo_data.met_file_params['{FILENAME}']}.met",
                        'LAT': meteo_data.met_file_params['{LAT}'],
                        'ALT': meteo_data.met_file_params['{ALT}'],}

        self._update_swp(self._get_args_dict(locals()), 'meteo')
        self._update_swp(meteo_params, 'meteo')
    ...
    def set_init_soil_moisture(self,
        ...
        self._update_swp(self._get_args_dict(locals()), 'soil_water')
    ...
    def set_drainage(self,
        ...
        drfil = drainage.file_name
        self.drainage = drainage

        self._update_swp(self._get_args_dict(locals()), 'drainage')
    ...
    def set_bottom_boundary(self,
        ...
        self._update_swp(self._get_args_dict(locals()), 'bott_bound')
    ...
    def set_heatflow(self,
        ...
        self._update_swp(self._get_args_dict(locals()), 'heat_flow')
    ...
    def set_solute_settings(self,
        ...
        self._update_swp(self._get_args_dict(locals()), 'solute')
