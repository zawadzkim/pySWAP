...
def _make_oxygenstress():
    ...
    general = ps.GeneralSettings(
        ...
        swyrvar=0,
        datefix="1993-12-31",
        inlist_csv=["pgrassdm", "grassdm", "pmowdm", "mowdm", "treddry", "tredwet"],
    )

    # %% Meteorological section
    metfile = pyswap.components.meteorology.MetFile(
        metfil="260.met", content=ps.testcase.load_met("oxygenstress")
    )
    ...
    grass_rwu_comp = pyswap.components.crop.CompensateRWUStress(
        swcompensate=1, swstressor=1, alphacrit=0.7, dcritrtz=16
    )
    ...
    soil_moisture = pyswap.components.soilwater.SoilMoisture(
        swinco=2,
        gwli=-15.0,
    )

    surface_flow = pyswap.components.soilwater.SurfaceFlow(
    ...
    dra_settings = pyswap.components.drainage.DraSettings(
        dramet=3, swdivd=1, cofani=[1.0, 1.0, 1.0, 1.0], swdislay=0
    )
    ...
    flux2 = pyswap.components.drainage.Flux(
        level_number=2,
        drares=50.0,
        infres=40.0,
        swallo=3,
    ...
    drafile = pyswap.components.drainage.DraFile(
        general=dra_settings, drfil="swap", drainageinfres=inf_res
    )
